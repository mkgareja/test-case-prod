...
export class ProjectUtils {
  ...
  public async getProjects(id) {
    const result =  await mysql.findAll(`${Tables.PROJECT} p
        ...
        `p.${ProjectTable.IS_DELETE} = 0 AND p.${ProjectTable.IS_ENABLE} = 1 AND  p.${ProjectTable.USERID} = ? GROUP BY p.${ProjectTable.ID}`,
        [id]);
  ...
  public async getTestRun(id) {
    const result = await mysql.findAll(Tables.TESTRUNS,
      [TestrunsTable.DATA,TestrunsTable.DESCRIPTION,TestrunsTable.CREATED_AT,TestrunsTable.NAME], `${TestrunsTable.IS_DELETE} = 0 AND ${TestrunsTable.IS_ENABLE} = 1 and ${TestrunsTable.ID} = ?`, [id]);
      if (result.length >= 0) {
        return result;
      } else {
        return false;
  ...
  public async getTestRuns(id) {
    const result = await mysql.findAll(Tables.TESTRUNS,
      [TestrunsTable.DATA,TestrunsTable.DESCRIPTION,TestrunsTable.CREATED_AT,TestrunsTable.NAME], `${TestrunsTable.IS_DELETE} = 0 AND ${TestrunsTable.IS_ENABLE} = 1 and ${TestrunsTable.PROJECTID} = ?`, [id]);
      if (result.length >= 0) {
        return result;
      } else {
        return false;
